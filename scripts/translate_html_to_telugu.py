...
TARGET_LANG = "te"
DEFAULT_CONFIG_PATH = Path(__file__).resolve().parents[1] / "config" / "media-config.properties"


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Translate HTML to Telugu using settings from properties file."
    )
    parser.add_argument(
        "--config",
...
def main() -> int:
    ...
    logger = logging.getLogger("translate_html_to_telugu")

    config_path = Path(args.config).expanduser().resolve()
    cfg = load_config(config_path)

    input_html_file = cfg.get("input_html_file", "").strip()
    ...
    if not input_html_file:
        logger.error("Missing 'input_html_file' in %s", config_path)
        return 1
    if not api_key:
    ...
    input_path = Path(input_html_file).expanduser().resolve()
    if not input_path.exists():
        logger.error("Input HTML file not found: %s", input_path)
        return 1
