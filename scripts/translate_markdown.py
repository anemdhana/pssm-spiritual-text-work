...
class _HtmlTranslatorParser(HTMLParser):
	def __init__(
		self,
		translator: callable,
		logger: logging.Logger,
	...
	def handle_data(self, data: str) -> None:
		if not data:
			return
		if self._skip_depth > 0 or data.isspace():
			self._parts.append(data)
			return
		try:
			self._parts.append(self._translator(data))
		except Exception as exc:  # noqa: BLE001
			self._logger.error("HTML text translation failed: %s", exc)
			self._parts.append(data)
...
def translate_html_file(
	...
	parser = _HtmlTranslatorParser(translator=_translate, logger=logger)
	parser.feed(html)
	parser.close()

	output_path.write_text(parser.translated_html(), encoding="utf-8")
